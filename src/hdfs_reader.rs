...
impl HdfsReader {
    ...
    pub async fn new(namenode_uri: &str, base_path: &str) -> Result<Self> {
        ...
        let client = Client::new(namenode_uri)
            .map_err(|e| anyhow!("Failed to connect to HDFS: {}", e))?;
                    ...
                    if let Ok(entry) = entry {
                        if entry.path().is_file() {
                            ...
                            if let Some(name) = filename.to_str() {
                                if name.starts_with("events") || name.contains("eventLog") || name.ends_with(".inprogress") {
    ...
    pub async fn read_events(&self, file_path: &str, app_id: &str) -> Result<Vec<SparkEvent>> {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            match serde_json::from_str::<Value>(line) {
                Ok(json_event) => {
                    match SparkEvent::from_json(&json_event, app_id) {
                        Ok(event) => events.push(event),
                        Err(e) => warn!("Failed to parse event at line {}: {}", line_num + 1, e),
                    }
                }
                Err(e) => warn!("Invalid JSON at line {} in {}: {}", line_num + 1, file_path, e),
            }
        }
    ...
    pub async fn read_application_events(&self, app_id: &str) -> Result<Vec<SparkEvent>> {
        ...
        info!("Total {} events loaded for application {}", all_events.len(), app_id);
        Ok(all_events)
    }
