...
async fn test_hdfs_to_duckdb_pipeline() -> Result<()> {
    ...
    let events = hdfs_reader.read_events(&event_file.to_string_lossy(), app_id).await?;
...
async fn test_batch_writer_simulation() -> Result<()> {
    ...
    for i in 0..2 {
        let event_json = create_sample_event_json(app_id, "SparkListenerApplicationStart", 1640000000000 + i);
    ...
    for i in 2..5 {
        let event_json = create_sample_event_json(app_id, "SparkListenerJobStart", 1640000000000 + i);
    ...
    // Process first file
    let events1 = hdfs_reader.read_events(&event_file_1.to_string_lossy(), app_id).await?;
    println!("Sending {} events from file 1", events1.len());
    for event in events1 {
        event_tx.send(event).map_err(|e| anyhow::anyhow!("Send error: {}", e))?;
    }
    
    // Process second file
    let events2 = hdfs_reader.read_events(&event_file_2.to_string_lossy(), app_id).await?;
    ...
    batch_writer_handle.await.map_err(|e| anyhow::anyhow!("Join error: {}", e))?;
...
async fn test_id_conflict_issue() -> Result<()> {
    ...
    for i in 1..=2 {  // IDs 1, 2 already exist
        let event = spark_history_server::storage::duckdb_store::SparkEvent {
            id: i,
...
async fn test_max_id_method() -> Result<()> {
    ...
    // Insert some events
    let mut events = Vec::new();
    for i in 10..13 {  // Start with ID 10 to test non-zero start
        let event = spark_history_server::storage::duckdb_store::SparkEvent {
            id: i,
