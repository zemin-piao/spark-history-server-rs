...
async fn test_40k_applications_scalability() -> Result<()> {
    ...
    println!("🚀 Starting scalability test: {} applications, {} events total", 
    ...
    println!("⏱️  Event generation: {:?} ({:.0} events/sec)", 
             gen_duration, TOTAL_EVENTS as f64 / gen_duration.as_secs_f64());
    ...
    println!("📦 Processing {} batches of up to {} events each", batches.len(), batch_size);
        ...
        let handle = tokio::spawn(async move {
            ...
            match result {
                Ok(()) => {
                    ...
                    println!("✅ Batch {}: {} events in {:?} ({:.0} events/sec)", 
                             batch_idx, batch_events.len(), batch_duration, throughput);
                    Ok(batch_events.len())
                }
                Err(e) => {
    ...
    println!("  📊 Total events processed: {}/{}", total_processed, TOTAL_EVENTS);
    ...
    println!("  📈 Estimated 40K app capacity: {:.0} events/sec", 
    ...
    assert_eq!(total_processed, TOTAL_EVENTS, "All events should be processed");
    assert!(overall_throughput > 5000.0, 
            "Should achieve >5000 events/sec throughput (got: {:.0})", overall_throughput);
...
async fn test_concurrent_writer_isolation() -> Result<()> {
    ...
    for writer_id in 0..NUM_WRITERS {
        ...
        let handle = tokio::spawn(async move {
            ...
            println!("Writer {} completed {} events in {:?}", 
    ...
    println!("✅ Concurrent writers test: {}/{} events processed", final_count, expected);
    assert_eq!(final_count as usize, expected, "All events should be processed without conflicts");
