...
use spark_history_server::{
    event_processor::EventProcessor, hdfs_reader::HdfsConfig, metadata_store::MetadataStore,
...
async fn test_new_event_log_file_detection() -> Result<()> {
    ...
    let hdfs_reader =
        spark_history_server::hdfs_reader::HdfsReader::new("", &hdfs_path.to_string_lossy())
            .await?;
    ...
    let metadata_store_arc = std::sync::Arc::new(
        spark_history_server::metadata_store::MetadataStore::new(&metadata_path).await?,
    );
    let (_, _event_rx): (
        tokio::sync::mpsc::UnboundedSender<spark_history_server::spark_events::SparkEvent>,
        _,
    ...
    let should_reload = metadata_store_arc
        .should_reload_file(&file2_path, file_info.size)
        .await;
    ...
    println!(
        "Manual test completed. Initial: {}, Final: {}, Should reload second file: {}",
