...
async fn test_duckdb_incremental_appends() -> Result<()> {
    ...
    assert!(
        !applications.is_empty(),
        "Should have applications available"
    );

    // Find an application that matches our test data or verify we have valid application data
    let has_valid_apps = applications
        .iter()
        .any(|app| !app.id.is_empty() && !app.name.is_empty());
    assert!(
        has_valid_apps,
        "Should have valid application data available"
    );
